...
cal_pct = function(dataframe,ctg.idx,num.idx,condition,condition.idx){
  ...
  dataframe[is.na(dataframe[,2]),2]=0
  dataframe$percent = dataframe[,2]/sum(dataframe[,2])*100

  colnames(dataframe)=c('index','value','percent')
  dataframe=ddply(dataframe,"index",function(d){
    sum_pct=sum(d$percent)
    data.frame(percent=sum_pct)
  })
